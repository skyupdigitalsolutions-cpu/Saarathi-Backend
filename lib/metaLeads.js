...
export function mapMetaFields(fieldData = [], extra = {}) {
  ...
  const monthlyIncomeRaw = fuzzy(
    ...
    "salary", "income", "whatisyourmonthlysalary", "monthlypay"
  );
  const monthlyIncome = parseIndianAmount(monthlyIncomeRaw);

  // Loan type (field or infer from campaign)
  const loanRaw = (fuzzy("loantype", "whichloan", "typeofloan", "loanrequire", "kindofloan", "loanfor") || extra.campaign || "").toLowerCase();
