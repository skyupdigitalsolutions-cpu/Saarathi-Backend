...
export function mapMetaFields(fieldData = [], extra = {}) {
  ...
  const amount = parseIndianAmount(
    fuzzy("howmuchloan", "loanamount", "loanrequire", "requiredloan", "amount")
  );

  // Monthly income
  const monthlyIncome = parseIndianAmount(
    fuzzy("monthlyinhand", "monthlyincome", "inhandsalary", "salary", "income")
  );

  // Loan type (field or infer from campaign)
  const loanRaw = (fuzzy("loantype", "whichloan", "typeofloan", "loanrequire") || extra.campaign || "").toLowerCase();
  ...
  // Employment type
  const empRaw = (fuzzy("employmenttype", "employment", "occupation", "jobtype") || "").toLowerCase();
  ...
  // Urgency
  const urgRaw = (fuzzy("urgency", "howsoon", "whendoyou", "timeline") || "").toLowerCase();
